    ...
    def process_payloads(self, year, payloads):
        ...
        for payload in payloads:
            ...
            if city not in self.data.keys():
                self.data[city] = AverageSlidingWindow()

            self.data[city].add(dt, payload.p2)
    ...
    def process(self, batch):
        ...
        for (k, v) in scores_curr.items():
            ...
            if k in scores_lastyear:
                ...
                curr_year_aqi = v
                res.append((k, last_year_aqi - curr_year_aqi,
                            last_year_aqi, curr_year_aqi))
        ...
        for i in range(1, topk + 1):
            if len(sort_res) >= i:
                ...
                topklist.append(ch.TopKCities(
                    position=i, city=res[0], averageAQIImprovement=res[1], currentAQI=res[2], previousAQI=res[3]))
