...
public class ZipEntryCollector {
    ...
    public boolean collect() throws Exception {
        ...
        while(line != null) {
            ...
            if(first) {
                ...
                var ds = parseLine(line, indexToSmart, normalized, raw);
                if(ds != null) {
                    if (!bl.collectState(ds)) {
                        return false;
                    }
                } 
                normalized.clear();
    ...
    private DriveState.Builder parseLine(String line, final HashMap<Integer, Integer> indexToSmart, HashMap<Integer, Long> normalized, HashMap<Integer, Long> raw) throws Exception {
        String[] parts = line.split(",", -1);
        if(parts.length == 0) {
            return null;
        }
